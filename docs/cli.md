...
Applies to **published 0.6.0**. `compare`, its exit codes and the JSON report's
comparison metadata are new in 0.6.0 and are not present in 0.5.1.
...
| `compare` | **New in 0.6.0.** Compare two saved JSON reports (older first), working-tree reports only. Reads files only — never rescans, verifies or contacts a provider. Results name the rule and identity digest; scanned paths are not echoed. |
...
| `help` | Print the help text. The version is a **flag**, `--version`; there is no `version` command, and `secretloop version` is rejected as unknown. |
...
`compare` and these codes are **new in 0.6.0** — 0.5.1 has no `compare` command.
It uses its own codes: `0` compared with nothing
...
The comparison metadata described here is **new in 0.6.0**: 0.5.1 does not emit
it.
