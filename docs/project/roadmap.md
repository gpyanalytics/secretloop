...
> **Status, September 2026.** Since this plan was locked, 0.1.x through 0.4.0 shipped
> (npm serves 0.4.0). Phase 0's documentation move is done; the planning pages
> now live under `docs/project/` and the decision records under
> `docs/decisions/`. Detection scope decisions taken since — opt-in entropy,
> one-layer decoding, archive depth one, the API-document entropy scope, archive
> coverage accounting and the consent gate — are recorded in
> [decisions](../decisions/README.md); the current open items are in
> [development](../development.md#open-items). The strategy, sequencing and
> out-of-scope table below stand.
...
- [x] Move `BACKLOG.md` and `MARKET.md` into `docs/` (now `docs/project/`)
