...
import {
  ...
  MEMBER_SEPARATOR, ArchiveListing } from "../src/archive";
...
function opened(b: Buffer, p: string, m: number): ArchiveListing {
  const o = openArchive(b, p, m);
  if (o === null || "notOpened" in o) throw new Error(`expected an opened listing, got ${JSON.stringify(o)}`);
  return o;
}
const refusedTotal = (l: ArchiveListing): number => Object.values(l.refused).reduce<number>((n, v) => n + (v ?? 0), 0);
...
test("stored and deflated members come back byte-exact, in central-directory order", () => {
  ...
  assert.deepStrictEqual(l.refused, {});
});

test("an empty archive (end record only) is a valid archive with no members", () => {
  const l = opened(buildZip([]), "e.zip", MAX);
  assert.deepStrictEqual(l.members, []);
});

test("directory entries are structure, not content, and are not counted as skipped", () => {
  const l = opened(buildZip([{ name: "a", data: Buffer.alloc(0), dir: true }, { name: "a/b.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["a/b.txt"]);
  assert.strictEqual(refusedTotal(l), 0);
});

test("encrypted entry (flag bit 0) is skipped and counted; never decrypted", () => {
  const l = opened(buildZip([{ name: "s.txt", data: secret, method: 0, flags: 1 }, { name: "r.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
  assert.deepStrictEqual(l.refused, { encrypted: 1 });
});

test("unsupported compression method is skipped and counted", () => {
  const l = opened(buildZip([{ name: "s.txt", data: secret, method: 12, payload: text("not bzip2 really") }, { name: "r.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
  assert.deepStrictEqual(l.refused, { "unsupported-compression": 1 });
});

test("unsafe member names are skipped and counted, one safe member survives", () => {
  const names = ["../s1", "/abs/s2", "C:\\abs\\s3", "nested/../../s4", "a/../s5", ""];
  const l = opened(buildZip([...names.map((name) => ({ name, data: secret })), { name: "safe/ok.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["safe/ok.txt"]);
  assert.deepStrictEqual(l.refused, { "unsafe-name": names.length });
});

test("duplicate normalized names: first wins, later occurrences skipped and counted", () => {
  const l = opened(buildZip([{ name: "dup.txt", data: README }, { name: "dup.txt", data: secret }, { name: ".\\dup.txt", data: secret }]), "x.zip", MAX);
  assert.strictEqual(l.members.length, 1);
  assert.ok(l.members[0].bytes.equals(README));
  assert.deepStrictEqual(l.refused, { "duplicate-name": 2 });
});

test("a member over the size cap is skipped as oversized before any inflate", () => {
  const big = Buffer.alloc(MAX + 1, 0x61);
  const l = opened(buildZip([{ name: "big.txt", data: big }, { name: "r.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
  assert.deepStrictEqual(l.refused, { oversized: 1 });
});
...
test("the decompression budget (100 x outer) stops processing; later members are never inflated", () => {
  ...
  const l = opened(z, "bomb.zip", MAX);
  assert.ok(l.members.every((m) => m.member !== "last.txt"), "member after the budget must not be offered");
  assert.strictEqual(l.enumeration.complete, false);
  assert.strictEqual(l.enumeration.reason, "decompression-budget");
  assert.ok(l.enumeration.declaredNotInspected >= 1, "a stopped ZIP walk knows how many declared entries it left");
});
...
test("the member cap: entry 10,001 onward is never processed and the parse terminates", () => {
  ...
  const l = opened(buildZip(entries), "many.zip", MAX);
  assert.deepStrictEqual(l.members, [], "empty entries carry nothing and the secret sits beyond the cap");
  assert.deepStrictEqual(l.enumeration, { complete: false, reason: "member-cap", declaredNotInspected: 6 });
  assert.strictEqual(l.empty, MAX_MEMBERS_PER_ARCHIVE, "the entries walked were all empty members");
});

test("a member whose name exceeds 1024 characters is skipped", () => {
  const l = opened(buildZip([{ name: "d/" + "x".repeat(1100), data: secret }, { name: "r.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
});

test("CRC mismatch skips the member", () => {
  const l = opened(buildZip([{ name: "s.txt", data: secret, crc: 0xdeadbeef }, { name: "r.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
  assert.deepStrictEqual(l.refused, { malformed: 1 });
});

test("declared size that the inflated output does not match skips the member", () => {
  const l = opened(buildZip([{ name: "s.txt", data: secret, usize: secret.length + 5 }, { name: "r.txt", data: README }]), "x.zip", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
});

test("truncated archive and corrupted end record are recognised containers that will not open", () => {
  ...
  assert.deepStrictEqual(openArchive(bad, "m.zip", MAX), { notOpened: "malformed" });
});
...
test("ZIP64 markers make the archive unsupported", () => {
  ...
  assert.deepStrictEqual(openArchive(z, "z64.zip", MAX), { notOpened: "unsupported-feature" });
});
...
test("symlink, hardlink and device entries are refused; GNU long-name and pax records are metadata; targets never read", () => {
  const t = buildTar([
  ...
  assert.deepStrictEqual(l.refused, { "non-regular-entry": 3 });
  assert.strictEqual(l.metadataEntries, 2);
});
...
test("data truncated inside a member skips that member; the complete one before it is kept", () => {
  ...
  const l = opened(cut, "x.tar", MAX);
  assert.deepStrictEqual(l.members.map((m) => m.member), ["a.txt"]);
  assert.deepStrictEqual(l.refused, { malformed: 1 });
  assert.deepStrictEqual(l.enumeration, { complete: false, reason: "truncated", declaredNotInspected: 0 });
});
...
test("a member declared larger than the cap is skipped and the walk continues", () => {
  ...
  const l = opened(t, "x.tar", 500_000); // cap below the member, above README
  assert.deepStrictEqual(l.members.map((m) => m.member), ["r.txt"]);
  assert.deepStrictEqual(l.refused, { oversized: 1 });
});
...
test("unsafe and duplicate names in a tar follow the same rules as zip", () => {
  ...
  assert.ok(l.members[0].bytes.equals(README));
  assert.deepStrictEqual(l.refused, { "unsafe-name": 2, "duplicate-name": 1 });
});
...
test("ustar prefix + name reconstructs a long path; two zero blocks alone are not a tar", () => {
  ...
  const l = opened(buildTar([{ name: long, data: secret }]), "x.tar", MAX);
...
test("an archive stored as a tar member is returned opaquely as bytes; the layer never opens it", () => {
  ...
  const l = opened(buildTar([{ name: "inner.zip", data: inner }]), "x.tar", MAX);
  assert.strictEqual(l.members.length, 1);
...
test("pure gzip: one member named from the outer path; FNAME is ignored", () => {
  const l = opened(buildGzip(secret, "../../etc/creds"), "logs/app.log.gz", MAX);
...
test("concatenated gzip members with different FNAMEs decode to ONE stream in order, one identity", () => {
  ...
  const l = opened(cat, "cases/x.gz", MAX);
  assert.strictEqual(l.members.length, 1);
...
test("truncated and corrupt gzip inputs are recognised containers that will not open; non-gzip text is not a candidate", () => {
  ...
  assert.deepStrictEqual(openArchive(corrupt, "c.gz", MAX), { notOpened: "malformed" });
...
test("a stream over the member cap is skipped as oversized (bounded by maxOutputLength, never fully allocated)", () => {
  ...
  const l = opened(buildGzip(big), "big.gz", MAX);
  assert.deepStrictEqual(l.members, []);
  assert.deepStrictEqual(l.refused, { oversized: 1 });
});
...
test("gzip of a ustar is tgz: tar member paths are the identities, the wrapper's FNAME never is", () => {
  ...
  const l = opened(buildGzip(t, "should-not-appear.tar"), "site.tar.gz", MAX);
...
test("a .tgz whose payload is not a tar is handled as pure gzip (content-driven)", () => {
  const l = opened(buildGzip(secret), "cases/odd.tgz", MAX);
  assert.strictEqual(l.containerKind, "gzip");
...
test("tgz intermediate may exceed the member cap while every member stays under it", () => {
  ...
  const l = opened(outer, "x.tgz", 500_000);
  assert.strictEqual(l.containerKind, "tgz");
...
test("gzip of a gzip is opaque: the inner stream is returned as bytes, never decoded", () => {
  ...
  const l = opened(buildGzip(inner), "x.gz.gz", MAX);
  assert.strictEqual(l.members.length, 1);
